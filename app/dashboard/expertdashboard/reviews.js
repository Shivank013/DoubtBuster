...
const Reviews = () => {
  ...
  return (
    <div className="">
      <h1 className="text-center font-bold text-[1.5rem] mt-5">
        Expert Reviews
      </h1>
      {isLoading ? (
        <div className="">
        ...
        <ul className=" flex gap-5 flex-wrap w-full justify-evenly mt-[2.5rem] mb-[5rem]">
          {reviews.map((review, index) => (
            <li
              className="bg-[#d8d0d035] border p-5 mb-8 w-[30%] flex flex-col gap-5 hover:scale-110 hover:shadow-gray-600 transition rounded-lg shadow-black shadow-xl"
              key={index}
            >
              <RiDoubleQuotesL className="text-[#b6cfc6]" size={50} />
              <p className="text-gray-500 -mt-8 ml-11 font-semibold text-[1.1rem]">
                {review.feedback}
              </p>
              <p className="text-black font-bold opacity-90 uppercase text-center">
                Feedback: {review.userName}
              </p>
